...
%% For any time step: Set boundary blocks
    ...
    if info.Choice{1,1} == "Pressure Gradient Specified"
        ...
        b = eval(info.B); m = eval(info.mu);
        T.x(:, 1, :, n+1) = ( 1.127 .* info.A{1} .* info.k{1} ) ./ ( m .* info.Delta(1) .* b );clear('b', 'm', 'P');
    end
    %% East
    if info.Choice{1,2} == "Pressure Gradient Specified"
        ...
        b = eval(info.B); m = eval(info.mu);
        T.x(:, info.s(1)+1, :, n+1) = ( 1.127 .* info.A{1} .* info.k{1} ) ./ ( m .* info.Delta(1) .* b );clear('b', 'm', 'P');
    end
    %% South
    if info.Choice{1,3} == "Pressure Gradient Specified" 
        ...
        b = eval(info.B); m = eval(info.mu);
        T.y(1, :, :, n+1) = ( 1.127 .* info.A{2} .* info.k{2} ) ./ ( m .* info.Delta(2) .* b );clear('b', 'm', 'P');
    end
    %% North
    if info.Choice{1,4} == "Pressure Gradient Specified" 
        ...
        b = eval(info.B); m = eval(info.mu);
        T.y(info.s(2)+1, :, :, n+1) = ( 1.127 .* info.A{2} .* info.k{2} ) ./ ( m .* info.Delta(2) .* b );clear('b', 'm', 'P');
    end
    %% Bottom
    if info.Choice{1,5} == "Pressure Gradient Specified" 
        ...
        b = eval(info.B); m = eval(info.mu);
        T.z(:, :, 1, n+1) = ( 1.127 .* info.A{3} .* info.k{3} ) ./ ( m .* info.Delta(3) .* b );clear('b', 'm', 'P');
    end
    %% Up
    if info.Choice{1,6} == "Pressure Gradient Specified" 
        ...
        b = eval(info.B); m = eval(info.mu);
        T.z(:, :, info.s(3)+1, n+1) = ( 1.127 .* info.A{3} .* info.k{3} ) ./ ( m .* info.Delta(3) .* b );clear('b', 'm', 'P');
    end
end